...
public class Tablero implements Serializable, Cloneable {
	 ...
	 */
	final private Carta[] cartasMesa;
	/**
	...
	String tirarCarta(int idjug, int idcarta) {//idjug 
		if (idjug == idJugActual) {
			if (compruebaCarta(idjug, idcarta) == null) {
				cartasMesa[getCartasMesaIndex()] = jugadores[idjug].tirarCarta(idcarta);
	 ...
	 */
	private int quienMata(Carta c1, Carta c2, Carta c3) {
		if (c3 == null) {
	...
	private boolean isCantado(char palo, int idjug) {
		return cantes[aQueBaza(idjug)][Utils.paloInt(palo)];
	}
	...
	private void marcarCante(char palo, int idjug) {
		cantes[aQueBaza(idjug)][Utils.paloInt(palo)] = true;
		if (palotriunfo == palo) {
	...
	/**
	 * @param idjug
	 * @return "" si no tiene cantes;
	 * el/los palos si tiene cantes y no estan cantados y los marca
	 */
	...
	/**
	 * relacion cartasMesa -> idjug
	 * cartasMesa[0] -> ultBaza
	 * cartasMesa[1] -> siguiente(ultBaza)
	 * cartasMesa[2] -> siguiente(siguiente(ultBaza))
	 * cartasMesa[3] -> siguiente(siguiente(siguiente(ultBaza)));
	...
	/**evalua cartasMesa y 
	 * devuelve el idjug del jugador que ha tirado la mejor carta
	 * @return
	 */
	 ...
	 * idJugActual debe ser el anterior a ultBaza en este punto, excepto en la primera jugada;
	 * decide el idJug del ganador y lo devuelve, actualiza ultBaza e idJugActual,
	 * recoge las cartas a su respectiva baza (la del idjug 0 o 1),
